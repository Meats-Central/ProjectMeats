...
def exception_handler(exc, context):
    ...
    if isinstance(exc, Http404):
        ...
        return Response(
            {"error": "Not Found", "details": str(exc)}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    if isinstance(exc, DatabaseError):
        ...
        # Check for read-only database errors
        is_readonly_error = (
            "readonly" in error_message
            or "read-only" in error_message
            or "read only" in error_message
            or "attempt to write a readonly database" in error_message
        )

        if is_readonly_error:
            logger.critical(
                f"Read-Only Database Error: {str(exc)}",
                extra={
                    "view": context.get("view").__class__.__name__
                    if context.get("view")
                    else "Unknown",
                    "error_type": "readonly_database",
                },
                exc_info=True,
            )
            return Response(
                {
                    ...
                    "details": (
                        ...
                        "Check permissions or contact administrator."
                    ),
                    "error_type": "readonly_database",
                },
