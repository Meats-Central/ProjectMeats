...
def exception_handler(exc, context):
    ...
    if isinstance(exc, DatabaseError):
        ...
        if is_readonly:
            logger.critical(
                f"Read-Only Database Error: {str(exc)} - This typically indicates file permission issues. "
                f"Check database file permissions or contact administrator.",
                extra=log_extra,
                exc_info=True,
            )
            return Response(
            ...
            logger.critical(
                f"Database Error: {str(exc)}", extra=log_extra, exc_info=True
            )
            return Response(
                {
