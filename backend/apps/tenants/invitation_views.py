...
class TenantInvitationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self) -> QuerySet[TenantInvitation]:
        ...
        admin_tenants = TenantUser.objects.filter(
            user=user, role__in=["admin", "owner"], is_active=True
    ...
    def get_serializer_context(self) -> dict[str, Any]:
        ...
        if hasattr(self.request, "tenant") and getattr(
        ...
        # Or from user's default tenant (first admin/owner tenant)
            ...
            tenant_user = (
                TenantUser.objects.filter(
                    user=self.request.user, role__in=["admin", "owner"], is_active=True
                )
    ...
    def resend(self, request, pk=None):
        ...
        try:
            ...
            logger.error(
                f"Failed to send invitation email on resend: {str(e)}", exc_info=True
            )
            ...
            response_data[
                "warning"
            ] = "Invitation expiration updated, but email notification failed to send"
...
def validate_invitation(request):
    ...
    if not token:
        return Response(
            {"error": "Token parameter is required"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if invitation.is_expired:
        ...
        invitation.save()
        return Response(
            {"error": "This invitation has expired"}, status=status.HTTP_400_BAD_REQUEST
        )
