...
class TenantInvitationAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        return (
            ...
            .select_related("tenant", "invited_by", "accepted_by")
        )
        ...
        if is_new and obj.status == "pending":
            try:
                if send_invitation_email(obj):
                    messages.success(
                        request, f"Invitation created and email sent to {obj.email}"
                    )
                else:
                    messages.warning(
                        request,
                ...
                messages.error(request, f"Invitation created but email error: {str(e)}")
    ...
    resend_invitation_emails.short_description = (
        "Resend invitation emails (extends expiration)"
    )
